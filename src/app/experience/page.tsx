...
const experiences = [
  {
    ...
    description: 'Sole technical hire who built the firm\'s data analytics capability from zero. Platform now used by 10 senior executives for daily compliance triage of 13,905 pension plans managing $500M+ in retirement assets, replacing manual spreadsheet workflows with automated risk scoring, predictive modeling, and real-time dashboards.',
    achievements: [
      'Developed 7 production ML models (GradientBoosting, RandomForest, XGBoost) with automated 3-model shootout selection and 5-fold cross-validation. Correction risk classifier achieves 0.911 AUC with 82% recall; churn model achieves 0.986 AUC with 92% recall; system generates 53,000 predictions per nightly run across 13,905 plans',
      'Built out-of-time backtesting framework (PY2023 to PY2024) validating that reviewing just 12% of plans catches 39% of failures, a 3.3x triage lift reducing manual review burden during testing season',
      'Built graph-based compliance risk models using NetworkX and Louvain community detection to identify risk propagation patterns across recordkeeper ecosystems, surfacing plan clusters where a single compliance failure signals systemic exposure',
      'Delivered full-stack compliance operations dashboard (Next.js, FastAPI, SQL Server) used daily by 10 senior executives for plan-level risk triage, with interactive graph network visualizations that surface compliance contagion patterns previously invisible in tabular reports',
      'Engineered executive reporting layer in Power BI with 40+ custom DAX measures modeling the full 8-stage compliance lifecycle; gave CFO first-ever visibility into workflow bottlenecks, reducing monthly board reporting prep by 80%',
      'Codified IRS regulatory logic (IRC 401(k)/401(m) dual-prong ADP/ACP testing, 416 Top Heavy, Safe Harbor classification) into an automated compliance engine with per-plan audit trail generation. Each missed ADP/ACP failure exposes plans to 10% excise tax; on a $5M plan, that is $500K in avoidable penalties',
    ],
