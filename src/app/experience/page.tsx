...
const experiences = [
  {
    ...
    description: 'Sole technical hire who built the firm\'s data analytics capability from zero. Platform now used by 10 senior executives for daily risk triage of 13,905 financial plans managing $500M+ in assets, replacing manual spreadsheet workflows with automated risk scoring, predictive modeling, and real-time dashboards.',
    achievements: [
      'Developed 7 production ML models (GradientBoosting, RandomForest, XGBoost) with automated 3-model shootout selection and 5-fold cross-validation. Risk classifier achieves 0.911 AUC with 82% recall; churn model achieves 0.986 AUC with 92% recall; system generates 53,000 predictions per nightly run across 13,905 plans',
      'Built out-of-time backtesting framework validating that reviewing just 12% of plans catches 39% of failures, a 3.3x triage lift reducing manual review burden',
      'Built graph-based risk propagation models using NetworkX and Louvain community detection to surface hidden clusters where a single failure signals systemic exposure across linked entities',
      'Delivered full-stack operations dashboard (Next.js, FastAPI, SQL Server) used daily by 10 senior executives for risk triage, with interactive graph network visualizations that surface patterns previously invisible in tabular reports',
      'Engineered executive reporting layer in Power BI with 40+ custom DAX measures modeling an 8-stage operational lifecycle; gave CFO first-ever visibility into workflow bottlenecks, reducing monthly board reporting prep by 80%',
      'Codified complex regulatory logic into an automated rules engine with per-entity audit trail generation. Each missed failure exposes the firm to penalties up to $500K per case',
    ],
